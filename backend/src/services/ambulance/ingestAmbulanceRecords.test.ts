...
import { ingestAmbulanceRecords, IngestionLogEntry } from "./ingestAmbulanceRecords";
...
test("happy path: returns all fixture records in one attempt", async () => {
  ...
  const result = await ingestAmbulanceRecords(new MockAmbulanceClient(), {
    idempotencyKey: "ambulance-test-1",
    timeoutMs: 1000,
    logger,
    trustLogger,
  });
...
test("trust spine: a completed ingestion process is logged exactly once with its transaction ID", async () => {
  const trustLogger = new FakeTrustLogger();
  const result = await ingestAmbulanceRecords(new MockAmbulanceClient(), {
    idempotencyKey: "ambulance-test-trust",
    timeoutMs: 1000,
    trustLogger,
  });
  ...
  assert.equal(trustLogger.records[0]?.idempotencyKey, "ambulance-test-trust");
  assert.equal(result.outcome, "success");
  if (result.outcome === "success") {
...
test("trust spine: replaying the same idempotencyKey reuses the same transaction ID", async () => {
  const trustLogger = new FakeTrustLogger();
  const first = await ingestAmbulanceRecords(new MockAmbulanceClient(), {
    idempotencyKey: "ambulance-test-replay",
    timeoutMs: 1000,
    trustLogger,
  });
  const second = await ingestAmbulanceRecords(new MockAmbulanceClient(), {
    idempotencyKey: "ambulance-test-replay",
    timeoutMs: 1000,
    trustLogger,
  });
...
test("failure path: a trust-log write failure fails the ingestion loudly instead of returning an unlogged result", async () => {
  ...
  await assert.rejects(
    () =>
      ingestAmbulanceRecords(new MockAmbulanceClient(), {
        idempotencyKey: "ambulance-test-log-failure",
        timeoutMs: 1000,
        trustLogger,
      }),
...
test("happy path: since filter narrows to records captured at/after the cutoff", async () => {
  ...
  const result = await ingestAmbulanceRecords(new MockAmbulanceClient(), {
    idempotencyKey: "ambulance-test-since",
...
test("failure path: format mismatch fails on the first attempt and is never retried", async () => {
  const { logs, logger } = collectLogs();
  const result = await ingestAmbulanceRecords(new MockAmbulanceClient({ failureMode: "format" }), {
    idempotencyKey: "ambulance-test-format",
    timeoutMs: 1000,
    maxAttempts: 3,
    backoffBaseMs: 0,
    logger,
...
test("failure path: connection failure retries up to the cap, logging every attempt", async () => {
  const { logs, logger } = collectLogs();
  const result = await ingestAmbulanceRecords(new MockAmbulanceClient({ failureMode: "connection" }), {
    idempotencyKey: "ambulance-test-connection",
    timeoutMs: 1000,
    maxAttempts: 3,
    backoffBaseMs: 0,
    logger,
...
test("failure path: timeout is caller-enforced, retries to the cap, and returns promptly", async () => {
  ...
  const result = await ingestAmbulanceRecords(new MockAmbulanceClient({ failureMode: "timeout" }), {
    idempotencyKey: "ambulance-test-timeout",
    timeoutMs: 100,
    maxAttempts: 2,
    backoffBaseMs: 10,
    logger,
