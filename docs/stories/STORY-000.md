...
**AI Healthcare Operations Command Centre** — An AI-powered operational intelligence platform for NHS to predict pressure, identify risks, and recommend interventions.
...
- **REQ-014** — The system must reduce operational decision-making time from 3 hours to under 1 hour.
...
Who this is for and what they are trying to get done. Take the roles from your own stories — they are written "As a <role>, I want …". Roles in your plan: healthcare analyst, data analyst, UI/UX designer, AI architect, data engineer, performance engineer.
...
- **REQ-013** — The system must flag data uncertainties for human review.
- **REQ-018** — The system must not make autonomous clinical decisions.
...
- **r0** Initial Data Ingestion and Basic Prediction — 3 stories
- **r1** AI Intelligence Layer — 1 story
- **r2** AI Decision Engine and Dashboard — 2 stories
- **r3** AI What-If Simulator and Governance — 2 stories
- **r4** Performance Optimization and Final Integration — 1 story
...
- **Data Integration Team** — owns STORY-001, STORY-002
- **AI Development Team** — owns STORY-006, STORY-012
- **UI/UX Designer** — owns STORY-007
- **AI Architect** — owns STORY-008
- **Data Engineer** — owns STORY-009
- **Performance Engineer** — owns STORY-010
...
- **REQ-001** (CONSTRAINT, must) — The system must ingest data from GP practice management software.
...
- **REQ-005** (CONSTRAINT, must) — The system must ingest data from Staffing rotas.
- **REQ-006** (CONSTRAINT, must) — The system must ingest data from Emergency department records.
- **REQ-007** (CONSTRAINT, must) — The system must ingest data from the 111 database.
- **REQ-008** (CONSTRAINT, must) — The system must ingest data from Discharge records.
- **REQ-009** (CONSTRAINT, must) — The system must ingest data from the Hospital bed occupancy record.
- **REQ-010** (FUNC, must) — The system must predict operational pressure within the next 4-24 hours.
- **REQ-011** (FUNC, must) — The system must provide recommendations for interventions to manage demand and capacity.
- **REQ-012** (FUNC, must) — The system must provide a concise briefing for the operational leadership team.
- **REQ-013** (SAFE, must) — The system must flag data uncertainties for human review.
- **REQ-014** (NFR, must) — The system must reduce operational decision-making time from 3 hours to under 1 hour.
- **REQ-015** (FUNC, should) — The system must provide an AI What-If Simulator for scenario planning.
- **REQ-016** (FUNC, must) — The system must display current and forecasted operational metrics on a dashboard.
- **REQ-017** (CONSTRAINT, must) — The system must use Claude Code for AI development.
- **REQ-018** (SAFE, must) — The system must not make autonomous clinical decisions.

## Your stories, in build order
**r0 · Initial Data Ingestion and Basic Prediction**
- STORY-001 — Enable data-driven insights from GP practice management software
